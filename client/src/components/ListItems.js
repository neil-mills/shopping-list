...
const ListItems = memo(
  ({ deleteItemFromList, listId, retailerId, items, userId }) => {
    const initialValues = {
      _id: [],
      name: [],
      categoryId: [],
    ...
    const formatErrors = (index, errors) => {
      setErrors(prevState => [...prevState, ...errors.map(({message,path}) => ({ index, message, path }))])
    }
      ...
      for (let _id of values._id) {
        ...
        try {
          if (_id) {
            ...
            if (response.data.createItem.length) {
